...
// 1. Add import
if (!code.includes("import { ThemePrimaryToneManager }")) {
  code = code.replace(
...
if (!code.includes("'theme-primary'")) {
    code = code.replace(
      "(['main-geo', 'main-2', 'main-3', 'main-6', 'theme-duo', 'streamer-premium', ] as AudioTheme[])",
      "(['main-geo', 'main-2', 'main-3', 'main-6', 'theme-duo', 'theme-primary', 'streamer-premium', ] as AudioTheme[])"
    );
    code = code.replace(
      "'streamer-sweet' | 'theme-duo';",
      "'streamer-sweet' | 'theme-duo' | 'theme-primary';"
    );
}

// 3. Add to init
if (!code.includes("if (theme === 'theme-primary') { ThemePrimaryToneManager.init(); }")) {
  code = code.replace(
...
functions.forEach(fn => {
  ...
  const linesToAdd = `
    if(currentTheme === 'theme-primary') { ThemePrimaryToneManager.${fn}(); return; }`.trim();
  
  if (code.includes(lineToFind) && !code.includes(`ThemePrimaryToneManager.${fn}()`)) {
