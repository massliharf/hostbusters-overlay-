...
imports = """import { ThemeWoodToneManager } from './ThemeWoodToneManager';
...
for hook in hooks:
    insert_str = f"""    if(currentTheme === 'theme-wood') {{ ThemeWoodToneManager.{hook}(); return; }}
    ...
    # We replace precisely `<hook>: () => {\n`
    pattern = re.compile(rf"({hook}: \(\) => {{\s*\n)")
    content = pattern.sub(rf"\1{insert_str}", content)

# Now manually do the UI replace
old_toggler_pattern = re.compile(r"\{\/\* THEME TOGGLER \*\/\}([\s\S]*?)<\/div>")

new_toggler = """{/* THEME TOGGLER */}
...
content = old_toggler_pattern.sub(new_toggler.replace('\\', '\\\\'), content, count=1)
