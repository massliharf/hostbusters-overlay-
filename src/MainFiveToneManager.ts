...
class MainFiveToneManagerClass {
  ...
  public async init() {
    await Tone.start();
    console.log("Tone.js MAIN 5: PIANO MINIMAL (Tasteful Option) Yüklendi! 🎹");
  }
  ...
  // 8. Word_Success: Tatlı Sade Piano
  win() {
    this.ctx();
    const now = this.t();

    // Çok sade, tatlı ve sıcak tek bir piano akoru + uzayan tail
    // C major -> yumuşak, mutlu ama abartısız
    this.pianoSampler.triggerAttackRelease(["C4", "E4", "G4"], 0.9, now, 0.78);

    // Hafif daha yüksek nota ile sıcak kapanış (echo ile uzasın)
    this.pianoSampler.triggerAttackRelease("C5", 2.2, now + 0.35, 0.52);

    // Çok hafif ikinci yumuşak dokunuş (dopamin için)
    setTimeout(() => {
      this.pianoSampler.triggerAttackRelease("E5", 1.8, this.t(), 0.38);
    }, 480);
  }
