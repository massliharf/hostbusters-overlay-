...
class MainFourToneManagerClass {
  // ==========================================
  ...
  private successSynth = new Tone.PolySynth(Tone.Synth, {
    ...
    envelope: { attack: 0.025, decay: 0.32, sustain: 0.22, release: 1.35 }
  }).connect(this.subCompressor);
  ...
  public async init() {
    await Tone.start();
    console.log("Tone.js MAIN 4: OPTION 4 (Sweet Fatsawtooth Fanfare Edition) Yüklendi! 🔥");
  }
  ...
  win() {
    ...
    // Ana tatlı melodik fanfare (ascending, major hissi)
    const melody = ["C4", "E4", "G4", "C5", "E5"];
    melody.forEach((note, i) => {
      const vel = 0.82 - (i * 0.06); // hafif azalan güç
      this.successSynth.triggerAttackRelease(
        note, 
        i < 3 ? 0.42 : 0.75, 
        now + i * 0.095, 
        vel
    ...
    // Hafif sparkle (keşif hissi)
    this.sparkleNoise.triggerAttackRelease(0.28, now + 0.25, 0.32);

    // Warm, uzun süren glow tail (tatlı kapanış)
    setTimeout(() => {
      this.glowSynth.triggerAttackRelease("G5", 2.1, this.t(), 0.38);
    }, 680);

    // Çok hafif bass destek (dopamin hissini güçlendirir, fazla baskın değil)
    this.winBass.triggerAttackRelease("C2", 0.65, now + 0.12, 0.45);
  }
...
export const MainFourToneManager = new MainFourToneManagerClass();
